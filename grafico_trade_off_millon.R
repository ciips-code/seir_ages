...
for (iso_country in paises) {
  ...
  dataPlot[[iso_country]]$m1 <- lm(dataPlot[[iso_country]]$trade_off_summary$muertes_primer_semestre_millon ~ dataPlot[[iso_country]]$trade_off_summary$costo_primer_semestre, data = dataPlot[[iso_country]]$trade_off_summary)
  dataPlot[[iso_country]]$m2 <- lm(dataPlot[[iso_country]]$trade_off_summary$muertes_segundo_semestre_millon ~ dataPlot[[iso_country]]$trade_off_summary$costo_segundo_semestre, data = dataPlot[[iso_country]]$trade_off_summary)
...
plot <- plot %>%
  add_trace(
    ...
    line = list(shape = 'spline', 
                color = '#fc8d59', 
                width = 2)) %>% add_annotations(x=max(dataPlot[["ARG"]]$trade_off_summary$costo_segundo_semestre*-1),
                                                y=min(predict(dataPlot[["ARG"]]$m2)),
  ...
  add_trace(
    ...
    line = list(shape = 'spline', 
                color = '#d7191c', 
                width = 2)) %>% add_annotations(x=max(dataPlot[["BRA"]]$trade_off_summary$costo_segundo_semestre*-1),
                                                y=min(predict(dataPlot[["BRA"]]$m2)),
  ...
  add_trace(
    ...
    line = list(shape = 'spline', 
                color = '#91bfdb', 
                width = 2)) %>% add_annotations(x=max(dataPlot[["MEX"]]$trade_off_summary$costo_segundo_semestre*-1),
                                                y=min(predict(dataPlot[["MEX"]]$m2)),
  ...
  add_trace(
    ...
    line = list(shape = 'spline', 
                color = '#91bfdb', 
                width = 2)) %>% add_annotations(x=max(dataPlot[["JAM"]]$trade_off_summary$costo_segundo_semestre*-1),
                                                y=min(predict(dataPlot[["JAM"]]$m2)),
                                                text=paste0("Jamaica (slope: ",format(round(dataPlot[["JAM"]]$m2[1]$coefficients[2]*-1,1)), nsmall=1,")"),
                                                showarrow = F) %>% 
  layout(title = '<br>Economic and epidemiological trade-off by country',
                                                                          yaxis = list(title = 'Daily deaths per millon (average)'),
