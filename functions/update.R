...
update <-  function(pais,diasDeProyeccion) {
  ...
  if (pais %in% c("BHS", "BRB","BLZ","BOL","DOM","ECU","GTM","GUY","HND","HTI","JAM","NIC","PAN","SLV","SUR","TTO", "VEN")) {
    ...
    data <- dataOWD[dataOWD$iso_code==pais, c("iso_code","date","new_deaths","new_cases")]
    data$new_deaths <- as.numeric(data$new_deaths)
    data$new_cases <- as.numeric(data$new_cases)
    data[is.na(data)] <- 0
